...
2. НЕ предлагай свои проценты для ОБЩЕГО матча — используй ТОЛЬКО {p1}% и {p2}% что даны
3. Для КАЖДОЙ из 7 карт пула дай конкретный ПРОЦЕНТ шанса на победу для каждой команды на этой карте (сумма = 100%). Базируй процент на общей форме команд, скорректированной под особенности каждой карты (темп, стиль атаки/защиты)
4. Не упоминай конкретных игроков по именам — фокус только на КОМАНДНОМ уровне (форма, темп, стабильность)
5. Карты — только из актуального пула, ВСЕ 7 карт должны быть в ответе
6. Всё строго на русском

Ответь ТОЛЬКО валидным JSON без markdown:
{{"verdict": "<1 строка — почему {winner_name} фаворит, с конкретной причиной из формы/статистики>", "maps_analysis": [{{"map": "<название карты>", "team1_pct": <целое 25-75>, "team2_pct": <целое 25-75, сумма с team1_pct=100>, "reason": "<короткая причина 1 строка>"}}], "key_factors": ["<факт1 про форму/статистику>", "<факт2>", "<факт3>"], "summary": "<2-3 предложения — итоговый разбор формы обеих команд и почему расклад {p1}/{p2}>"}}"""
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                # Нормализуем проценты по картам — гарантируем сумму = 100
                for m in result.get("maps_analysis", []):
                    t1p = m.get("team1_pct")
                    t2p = m.get("team2_pct")
                    if t1p is None or t2p is None:
                        m["team1_pct"], m["team2_pct"] = 50, 50
                        continue
                    total = t1p + t2p
                    if total != 100 and total > 0:
                        m["team1_pct"] = round(t1p / total * 100)
                        m["team2_pct"] = 100 - m["team1_pct"]

                # ВАЖНО: гарантируем что общие проценты матча не изменились
